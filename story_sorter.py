...
#lambda acts as a function passing through the string with the first index sliced off if the first index is " or ( and ignore case
sentences = sorted(sentences, key=lambda s: s[1:].lower() if s.startswith('"') or s.startswith('(') else s[2:].lower() if s.startswith('--') else s.lower())
